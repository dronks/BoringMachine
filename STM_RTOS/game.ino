...
void gameOver()
{
  ...
  if (endGameTime > bestGameTime)
  {
     ...
     //EEPROMWriteInt(bestGameTimeAddr, bestGameTime);
     uint16 DateWrite = bestGameTime / 1000;
     //EEPROM.write(bestGameTimeAddr, DateWrite);
     EEPROMWriteInt(bestGameTimeAddr, DateWrite); 
  }
...
void setup_game()
{
  ...
  //uint16 Data;
  //EEPROM.read(bestGameTimeAddr, &Data);
  //bestGameTime = Data * 1000;
}
...
void main_game()
{
  ...
  switch (gameStatus)
  {
     case MAIN_MENU:
       ...
       else if (button == BUTTON_BACK) {mainMode = 0; firstGame = 0; lcd.clear();} 
       break;
     case IN_GAME:
